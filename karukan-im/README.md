...
入力と同時にかな漢字変換の結果をプリエディットへリアルタイム表示します（Spaceを押さずに変換が進む）。`Ctrl+Shift+L` でON/OFFを切り替えられ、既定では `live_conversion = true` で有効です。macOS版ではキー入力を即時表示し、変換済みの接頭部分を保ったまま新しい読みを末尾へ追加します。入力が150ms止まった後に軽量モデルの結果を非同期反映し、全文がかなと漢字の間で行き来するちらつきを抑えます。
