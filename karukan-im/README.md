...
deferred_composing_chunk_len = 120 # macOS等の遅延ライブ変換で1回に扱う最大文字数（長文の文脈を維持）
...
入力と同時にかな漢字変換の結果をプリエディットへリアルタイム表示します（Spaceを押さずに変換が進む）。`Ctrl+Shift+L` でON/OFFを切り替えられ、既定では `live_conversion = true` で有効です。macOS版ではキー入力直後は入力バッファどおりの読みを表示し、入力が150ms止まった後に軽量モデルの結果を非同期反映します。直前の変換結果を新しい読みの前へ継ぎ足さないため、未入力の文字が入力中の単語の手前へ残ることはありません。

長文入力でもレイテンシを一定に保つため、変換中のバッファを内部チャンクに分割し、編集した箇所だけを再変換します。同期変換では `composing_chunk_len`（既定30）、macOS等のキー処理から分離した遅延変換では `deferred_composing_chunk_len`（既定120）を上限にします。遅延変換はより長い文脈を一度にモデルへ渡し、語中の固定長境界による精度低下を抑えます。チャンクはユーザーには連続した1つのプリエディットとして見えます。記号・数字の連続は日本語とは別チャンクに分けてそのまま通すため、`123456` のような並びが変換で崩れることはありません。かなだけの読みからモデルが推測した数字はライブ表示へ採用せず、意図した読み上げ数字への変換はSpace後の候補から選べます。
